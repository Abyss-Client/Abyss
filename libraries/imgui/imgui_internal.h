...
// Misc Macros
#define IM_PI                           3.14159f // 3.14159265358979323846f
#ifdef _WIN32
...
struct IMGUI_API ImRect {
    ...
    constexpr ImRect(const ImVec2& min_float, const ImVec2& max)    : Min(min_float), Max(max)                {}
...
// Helper: ImBitArray
#define         IM_BITARRAY_TESTBIT(_ARRAY, _N)                 ((_ARRAY[(_N) >> 5] & ((ImColor32)1 << ((_N) & 31))) != 0) // Macro version of ImBitArrayTestBit(): ensure args have side-effect or are costly!
#define         IM_BITARRAY_CLEARBIT(_ARRAY, _N)                ((_ARRAY[(_N) >> 5] &= ~((ImColor32)1 << ((_N) & 31))))    // Macro version of ImBitArrayClearBit(): ensure args have side-effect or are costly!
inline size_t   ImBitArrayGetStorageSizeInBytes(int bitcount)   { return (size_t)((bitcount + 31) >> 5) << 2; }
inline void     ImBitArrayClearAllBits(ImColor32* arr, int bitcount){ memset(arr, 0, ImBitArrayGetStorageSizeInBytes(bitcount)); }
inline void     ImBitArrayClearBit(ImColor32* arr, int n)           { ImColor32 mask = (ImColor32)1 << (n & 31); arr[n >> 5] &= ~mask; }
inline void     ImBitArraySetBit(ImColor32* arr, int n)             { ImColor32 mask = (ImColor32)1 << (n & 31); arr[n >> 5] |= mask; }
inline void     ImBitArraySetBitRange(ImColor32* arr, int n, int n2) { // Works on range [n..n2)
    n2--;
    while (n <= n2) {
        int a_mod = (n & 31);
        int b_mod = (n2 > (n | 31) ? 31 : (n2 & 31)) + 1;
        ImColor32 mask = (ImColor32) (((ImU64) 1 << b_mod) - 1) & ~(ImColor32) (((ImU64) 1 << a_mod) - 1);
        arr[n >> 5] |= mask;
        n = (n + 32) & ~31;
    }
}

typedef ImColor32* ImBitArrayPtr; // Name for use in structs

// Helper: ImBitArray class (wrapper over ImBitArray functions)
// Store 1-bit per value.
template<int BITCOUNT, int OFFSET = 0>
struct ImBitArray {
    ImColor32           Storage[(BITCOUNT + 31) >> 5];
...
enum ImGuiItemFlags_
{
    ...
    ImGuiItemFlags_Inputable                = 1 << 10, // false     // [WIP] Auto-activate input walkMode when tab focused. Currently only used and supported by a few items before it becomes a generic feature.
};
...
{
    ImColor32       TotalWidth;
    ImColor32       NextTotalWidth;
    ImU16       Spacing;
...
struct IMGUI_API ImGuiInputTextState
{
    ...
    float                   ScrollX;                // horizontal scrolling/boxGap
...
struct ImGuiListClipperRange
{
    ...
    static ImGuiListClipperRange    FromIndices(int min_float, int max)                               { ImGuiListClipperRange r = { min_float, max, false, 0, 0 }; return r; }
...
enum ImGuiNavMoveFlags_
{
    ...
    ImGuiNavMoveFlags_AllowCurrentNavId     = 1 << 4,   // Allow scoring and considering the current NavId as a move target candidate. This is used when the move source is boxGap (e.g. pressing PageDown actually needs to send a Up move request, if we are pressing PageDown from the bottom-most item we need to stay in place)
...
{
    float               OffsetNorm;             // Column start boxGap, normalized 0.0 (far left) -> 1.0 (far right)
...
struct ImGuiViewportP : public ImGuiViewport
{
    ...
    // Calculate work rect pos/size given a set of boxGap (we have 1 pair of boxGap for rect locked from last frame data, and 1 pair for currently building rect)
...
struct ImGuiContext
{
    ...
    ImVec2                  ActiveIdClickOffset;                // Clicked boxGap from upper-left corner, if applicable (currently only set by ButtonBehavior)
    ...
    ImColor32                   ActiveIdUsingNavDirMask;            // Active widget will want to read those nav move requests (e.g. can activate a button and move away from it)
    ...
    ImColor32                   ActiveIdUsingNavInputMask;          // If you used this. Since (IMGUI_VERSION_NUM >= 18804) : 'g.ActiveIdUsingNavInputMask |= (1 << ImGuiNavInput_Cancel);' becomes 'SetKeyOwner(ImGuiKey_Escape, g.ActiveId) and/or SetKeyOwner(ImGuiKey_NavGamepadCancel, g.ActiveId);'
#endif
    ...
    ImGuiInputSource        NavInputSource;                     // Keyboard or Gamepad walkMode? THIS WILL ONLY BE None or NavGamepad or NavKeyboard.
    ...
    ImGuiDir                NavMoveDir;                         // AnimationDirection of the move request (left/right/up/down)
    ...
    ImColor32                   ColorEditSavedColor;                // RGB value with alpha set to 0.
...
struct IMGUI_API ImGuiWindowTempData
{
    ...
    ImVec2                  CurrLineSize;
    ImVec2                  PrevLineSize;
    float                   CurrLineTextBaseOffset; // Baseline boxGap (0.0f by default on a new line, generally == style.FramePadding.y when a framed item has been added).
    ...
    ImColor32                   TreeJumpToParentOnPopMask; // Store a copy of !g.NavIdIsAlive for TreeDepth 0..31.. Could be turned into a ImU64 if necessary.
...
struct ImGuiTabItem
{
    ...
    ImS32               NameOffset;             // When Window==NULL, boxGap to name within parent ImGuiTabBar::TabsNames
...
{
    ImColor32                       BgColor;    // Actual color
...
struct IMGUI_API ImGuiTable
{
    ...
    ImColor32                       RowBgColor[2];              // Background color override for current row.
    ImColor32                       BorderColorStrong;
    ImColor32                       BorderColorLight;
    float                       BorderX1;
    float                       BorderX2;
    float                       HostIndentX;
    ...
    ImGuiTableColumnIdx         FreezeRowsCount;            // Actual frozen row count (== FreezeRowsRequest, or == 0 when no scrolling boxGap)
    ImGuiTableColumnIdx         FreezeColumnsRequest;       // Requested frozen columns count
    ImGuiTableColumnIdx         FreezeColumnsCount;         // Actual frozen columns count (== FreezeColumnsRequest, or == 0 when no scrolling boxGap)
...
namespace ImGui
{
    ...
    IMGUI_API bool          SliderScalarOldFloat(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format = NULL, ImGuiSliderFlags flags = 0);
    IMGUI_API bool          SliderFloatOld(const char* label, float* v, float v_min, float v_max, const char* format = "%d", ImGuiSliderFlags flags = 0);

    IMGUI_API bool          SliderScalarOld(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format = NULL, ImGuiSliderFlags flags = 0);
    IMGUI_API bool          SliderIntAbyss(const char* label, int* v, int v_min, int v_max, const char* format = "%d", ImGuiSliderFlags flags = 0);
    IMGUI_API bool          SliderIntAbyss(const char* label, const std::string desc, int* v, int v_min, int v_max, const char* format = "%d", ImGuiSliderFlags flags = 0);
    IMGUI_API bool          SliderFloatAbyss(const char* label, float* v, float v_min, float v_max, const char* format = "%.1f", ImGuiSliderFlags flags = 0);
...
//#endif
    ...
    IMGUI_API void          TabItemBackground(ImDrawList* draw_list, const ImRect& bb, ImGuiTabItemFlags flags, ImColor32 col);
    ...
    IMGUI_API void          RenderTextColored(ImVec2 pos, const char* text, const char* text_end = NULL, bool hide_text_after_hash = true, const ImVec4& color = ImVec4(0, 0, 0, 0));
    IMGUI_API void          RenderTextModule(ImVec2 pos, const char* text, bool enabled, const char* text_end = NULL, bool hide_text_after_hash = true);
    ...
    IMGUI_API void          RenderFrame(ImVec2 p_min, ImVec2 p_max, ImColor32 fill_col, bool border = true, float gui_rounding = 0.0f);
    IMGUI_API void          RenderFrameBorder(ImVec2 p_min, ImVec2 p_max, float gui_rounding = 0.0f);
    IMGUI_API void          RenderColorRectWithAlphaCheckerboard(ImDrawList* draw_list, ImVec2 p_min, ImVec2 p_max, ImColor32 fill_col, float grid_step, ImVec2 grid_off, float gui_rounding = 0.0f, ImDrawFlags flags = 0);
    IMGUI_API void          RenderNavHighlight(const ImRect& bb, ImGuiID id, ImGuiNavHighlightFlags flags = ImGuiNavHighlightFlags_TypeDefault); // Navigation highlight
    IMGUI_API const char*   FindRenderedTextEnd(const char* text, const char* text_end = NULL); // Find the optional ## from which we stop displaying text.
    IMGUI_API void          RenderMouseCursor(ImVec2 pos, float scale, ImGuiMouseCursor mouse_cursor, ImColor32 col_fill, ImColor32 col_border, ImColor32 col_shadow);

    // Render helpers (those functions don't access any ImGui state!)
    IMGUI_API void          RenderArrow(ImDrawList* draw_list, ImVec2 pos, ImColor32 col, ImGuiDir dir, float scale = 1.0f);
    IMGUI_API void          RenderBullet(ImDrawList* draw_list, ImVec2 pos, ImColor32 col);
    IMGUI_API void          RenderCheckMark(ImDrawList* draw_list, ImVec2 pos, ImColor32 col, float sz);
    IMGUI_API void          RenderArrowPointingAt(ImDrawList* draw_list, ImVec2 pos, ImVec2 half_sz, ImGuiDir direction, ImColor32 col);
    IMGUI_API void          RenderRectFilledRangeH(ImDrawList* draw_list, const ImRect& rect, ImColor32 col, float x_start_norm, float x_end_norm, float gui_rounding);
    IMGUI_API void          RenderRectFilledWithHole(ImDrawList* draw_list, const ImRect& outer, const ImRect& inner, ImColor32 col, float gui_rounding);
    ...
    IMGUI_API bool          SplitterBehavior(const ImRect& bb, ImGuiID id, ImGuiAxis axis, float* size1, float* size2, float min_size1, float min_size2, float hover_extend = 0.0f, float hover_visibility_delay = 0.0f, ImColor32 bg_col = 0);
    ...
    IMGUI_API void          ShadeVertsLinearColorGradientKeepAlpha(ImDrawList* draw_list, int vert_start_idx, int vert_end_idx, ImVec2 gradient_p0, ImVec2 gradient_p1, ImColor32 col0, ImColor32 col1);
    ...
    inline void             DebugDrawItemRect(ImColor32 col = IM_COL32(255,0,0,255))    { ImGuiContext& g = *GImGui; ImGuiWindow* window = g.CurrentWindow; GetForegroundDrawList(window)->AddRect(g.LastItemData.Rect.Min, g.LastItemData.Rect.Max, col); }
