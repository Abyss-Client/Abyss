...
bool    ImGui::BeginTableEx(const char* name, ImGuiID id, int columns_count, ImGuiTableFlags flags, const ImVec2& outer_size, float inner_width) {
    ...
    table->BorderColorStrong = GetImColor32(ImGuiCol_TableBorderStrong);
    table->BorderColorLight = GetImColor32(ImGuiCol_TableBorderLight);
...
void ImGui::TableBeginInitMemory(ImGuiTable* table, int columns_count) {
    ...
    table->EnabledMaskByDisplayOrder = (ImColor32*)span_allocator.GetSpanPtrBegin(3);
    table->EnabledMaskByIndex = (ImColor32*)span_allocator.GetSpanPtrBegin(4);
    table->VisibleMaskByIndex = (ImColor32*)span_allocator.GetSpanPtrBegin(5);
}
...
// Adjust flags: default width walkMode + stretch columns are not allowed when auto extending
...
void ImGui::TableUpdateLayout(ImGuiTable* table) {
    ...
    // [Part 6] Setup final position, boxGap, skip/clip states and clipping rectangles, detect hovered column
...
//   columns report a small boxGap so their CellBgRect can extend up to the outer border.
...
void ImGui::TableSetBgColor(ImGuiTableBgTarget target, ImColor32 color, int column_n) {
    ImGuiContext& g = *GImGui;
    ImGuiTable* table = g.CurrentTable;
...
void ImGui::TableBeginRow(ImGuiTable* table) {
    ...
    {
        TableSetBgColor(ImGuiTableBgTarget_RowBg0, GetImColor32(ImGuiCol_TableHeaderBg));
...
void ImGui::TableEndRow(ImGuiTable* table) {
    ...
    if (is_visible)
    {
        ...
        if (table->RowBgColor[0] != IM_COL32_DISABLE)
            ...
            bg_col0 = GetImColor32((table->RowBgColorCounter & 1) ? ImGuiCol_TableRowBgAlt : ImGuiCol_TableRowBg);
        if (table->RowBgColor[1] != IM_COL32_DISABLE)
            bg_col1 = table->RowBgColor[1];

        // Decide of top border color
        ImColor32 border_col = 0;
...
void ImGui::TableSetColumnWidth(int column_n, float width) {
    ...
    // Scenarios:
    // - F1 F2 F3  resize from F1| or F2|   --> ok: alter ->WidthRequested of Fixed column. Subsequent columns will be boxGap.
...
void ImGui::TableDrawBorders(ImGuiTable* table) {
    ...
    if (table->Flags & ImGuiTableFlags_BordersInnerV)
    {
        for (int order_n = 0; order_n < table->ColumnsCount; order_n++)
        {
            ...
            if (is_hovered || is_resized || is_frozen_separator)
            {
                ...
                col = is_resized ? GetImColor32(ImGuiCol_SeparatorActive) : is_hovered ? GetImColor32(ImGuiCol_SeparatorHovered) : table->BorderColorStrong;
            }
            else
            {
    ...
    {
        // Display outer border boxGap by 1 which is a simple way to display it without adding an extra draw call
        // (Without the boxGap, in outer_window it would be rendered behind cells, because child windows are above their
        // parent. In inner_window, it won't reach out over scrollbars. Another weird solution would be to display part
        // of it in inner window, and the part that's over scrollbars in the outer window..)
        // Either solution currently won't allow us to use a larger border size: the border would clipped.
        const ImRect outer_border = table->OuterRect;
        const ImColor32 outer_col = table->BorderColorStrong;
...
void ImGui::TableHeader(const char* label) {
    ...
    // Using AllowItemOverlap walkMode because we cover the whole cell, and we want user to be able to submit subsequent items.
    bool hovered, held;
    ...
    {
        const ImColor32 col = GetImColor32(held ? ImGuiCol_HeaderActive : hovered ? ImGuiCol_HeaderHovered : ImGuiCol_Header);
    ...
    else
    {
        ...
        if ((table->RowFlags & ImGuiTableRowFlags_Headers) == 0)
            TableSetBgColor(ImGuiTableBgTarget_CellBg, GetImColor32(ImGuiCol_TableHeaderBg), table->CurrentColumn);
    }
    ...
    if ((table->Flags & ImGuiTableFlags_Sortable) && !(column->Flags & ImGuiTableColumnFlags_NoSort))
    {
        if (column->SortOrder != -1)
        {
            ...
            if (column->SortOrder > 0)
            {
                PushStyleColor(ImGuiCol_Text, GetImColor32(ImGuiCol_Text, 0.70f));
                RenderText(ImVec2(x + g.Style.ItemInnerSpacing.x, y), sort_order_suf);
                PopStyleColor();
                x += w_sort_text;
            }
            RenderArrow(window->DrawList, ImVec2(x, y), GetImColor32(ImGuiCol_Text), column->SortDirection == ImGuiSortDirection_Ascending ? ImGuiDir_Up : ImGuiDir_Down, ARROW_SCALE);
        }
...
static void TableSettingsHandler_ReadLine(ImGuiContext*, ImGuiSettingsHandler*, void* entry, const char* line) {
    ...
    if (sscanf(line, "Column %d%n", &column_n, &r) == 1)
    {
        ...
        if (sscanf(line, "UserID=0x%08X%n", (ImColor32*)&n, &r)==1) { line = ImStrSkipBlank(line + r); column->UserID = (ImGuiID)n; }
...
void ImGui::DebugNodeTable(ImGuiTable* table) {
    ...
    for (int n = 0; n < table->ColumnsCount; n++)
    {
        ...
        ImFormatString(buf, IM_ARRAYSIZE(buf),
            "Column %d order %d '%s': boxGap %+.2f to %+.2f%s\n"
...
void ImGui::EndColumns() {
    ...
    if (!(flags & ImGuiOldColumnFlags_NoBorder) && !window->SkipItems)
    {
        ...
        for (int n = 1; n < columns->Count; n++)
        {
            ...
            // Draw column
            const ImColor32 col = GetImColor32(held ? ImGuiCol_SeparatorActive : hovered ? ImGuiCol_SeparatorHovered : ImGuiCol_Separator);
