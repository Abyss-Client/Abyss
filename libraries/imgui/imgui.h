...
#endif
#include "../../src/me/sleepyfish/abyss/modules/module.h"
...
struct ImFontGlyph;                 // A single font glyph (code point + coordinates within in ImFontAtlas + boxGap)
...
typedef unsigned int        ImColor32;  // 32-bit unsigned integer (often used to store packed colors)
...
namespace ImGui
{
    static inline std::string erters = "y";
    //void hotkey2(const char* label, KeyBind& key, float samelineOffset, const ImVec2& size);
    ...
    // - For each independent axis of 'size': ==0.0f: use remaining host window size / >0.0f: fixed size / <0.0f: use remaining window size minus abs(size) / Each axis can use a different walkMode, e.g. ImVec2(0,400).
    ...
    // custom made (sleepy : sleepy)
    IMGUI_API bool          ModuleSectionSleepy(Module* mod, ImVec2 size, ImGuiWindowFlags flags = 0);
    IMGUI_API bool          ModuleSectionSleepy(const char* str_id, ImVec2 size, bool* ratEnabled, bool border, ImGuiWindowFlags flags = 0);
    IMGUI_API bool          ModuleSectionSleepy(const char* str_id, std::string desc, ImVec2 size, bool* ratEnabled, bool border, ImGuiWindowFlags flags = 0);

    IMGUI_API bool          BeginChildSleepy(const char* str_id, const ImVec2& size = ImVec2(0, 0), bool border = false, ImGuiWindowFlags flags = 0);
    IMGUI_API bool          BeginChildSleepy(const char* str_id, int &width, int &height, bool border = false, ImGuiWindowFlags flags = 0);
    IMGUI_API bool          BeginChildSleepy(const char* str_id, float &width, float &height, bool border = false, ImGuiWindowFlags flags = 0);

    IMGUI_API void          EndChildSleepy(int offset);
    IMGUI_API void          EndChildSleepyC(int offsetX, int offsetY);
    IMGUI_API void          EndChildSleepyH();
    // custom made (sleepy : sleepy)
    ...
    IMGUI_API void          SetScrollFromPosX(float local_x, float center_x_ratio = 0.5f);  // adjust scrolling amount to make given position visible. Generally GetCursorStartPos() + boxGap to compute a valid position.
    IMGUI_API void          SetScrollFromPosY(float local_y, float center_y_ratio = 0.5f);  // adjust scrolling amount to make given position visible. Generally GetCursorStartPos() + boxGap to compute a valid position.

    // Parameters stacks (shared)
    IMGUI_API void          PushFont(ImFont* font);                                         // use NULL as a shortcut to push default font
    IMGUI_API void          PopFont();
    IMGUI_API void          PushStyleColor(ImGuiCol idx, ImColor32 col);                        // modify a style color. always use this if you modify the style after NewFrame().
    ...
    IMGUI_API void          PushButtonRepeat(bool repeat);                                  // in 'repeat' walkMode, Button*() functions return repeated true in a typematic manner (using io.KeyRepeatDelay/io.KeyRepeatRate setting). Note that you can call IsItemActive() after any Button() to tell if the button is held in the current frame.
    ...
    IMGUI_API ImColor32         GetImColor32(ImGuiCol idx, float alpha_mul = 1.0f);              // retrieve given style color with style alpha applied and optional extra alpha multiplier, packed as a 32-bit value suitable for ImDrawList
    IMGUI_API ImColor32         GetImColor32(const ImVec4& col);                                 // retrieve given color with style alpha applied, packed as a 32-bit value suitable for ImDrawList
    IMGUI_API ImColor32         GetImColor32(ImColor32 col);                                         // retrieve given color with style alpha applied, packed as a 32-bit value suitable for ImDrawList
    ...
    //    Window-local coordinates:   SameLine(), GetCursorPos(), SetOffset(), GetCursorStartPos(), GetContentRegionMax(), GetWindowContentRegion*(), PushTextWrapPos()
    //    Absolute coordinate:        GetCursorScreenPos(), SetCursorScreenPos(), all ImDrawList:: functions.
    IMGUI_API void          Separator();                                                    // separator, generally horizontal. inside a menu bar or in horizontal layout walkMode, this becomes a vertical separator.
    ...
    IMGUI_API ImVec2        GetOffsetPos();                                                 // cursor position in window coordinates (relative to window position)
    IMGUI_API float         GetOffsetX();                                                   //   (some functions are using window-relative coordinates, such as: GetCursorPos, GetCursorStartPos, GetContentRegionMax, GetWindowContentRegion* etc.
    IMGUI_API float         GetOffsetY();                                                   //    other functions such as GetCursorScreenPos or everything in ImDrawList::
    IMGUI_API void          SetOffset(const ImVec2& local_pos);                             //    are using the main, absolute coordinate system.
    IMGUI_API void          SetOffset(float local_x, float local_y);                        //    are using the main, absolute coordinate system.
    IMGUI_API void          SetOffsetX(float local_x);                                      //    GetWindowPos() + GetCursorPos() == GetCursorScreenPos() etc.)
    IMGUI_API void          SetOffsetY(float local_y);                                      //
    IMGUI_API ImVec2        GetCursorStartPos();                                            // initial cursor position in window coordinates
    IMGUI_API ImVec2        GetCursorScreenPos();                                           // cursor position in absolute coordinates (useful to work with ImDrawList API). generally top-left == GetMainViewport()->Pos == (0,0) in single viewport walkMode, and bottom-right == GetMainViewport()->Pos+Size == io.DisplaySize in single-viewport walkMode.
    ...
    IMGUI_API void          Text(const char* fmt, const ImVec4& col, ...)                   IM_FMTARGS(2); // shortcut for PushStyleColor(ImGuiCol_Text, col); Text(fmt, ...); PopStyleColor();
    ...
    IMGUI_API bool          CheckboxModule(Module* mod);
    IMGUI_API bool          CheckboxModule(const char* label, bool* v);
    IMGUI_API bool          Checkbox(const char* label, std::string desc, bool* v);
    IMGUI_API bool          CheckSliderAbyss(const char* label, bool* v);
    IMGUI_API bool          CheckSliderAbyss(const char* label, std::string desc, bool* v);
    ...
    IMGUI_API bool          SliderScalarSleepy(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags);
    ...
    IMGUI_API bool          ColorPicker3(const char* label, float col[4], ImGuiColorEditFlags flags = 0);
    IMGUI_API bool          ColorPicker4(const char* label, float col[4], ImGuiColorEditFlags flags = 0, const float* ref_col = NULL);
    
    IMGUI_API ImVec4        ColorPickerSmellon(const char* name, float color[4], bool renderName, ImGuiColorEditFlags flags = 0);
    ...
    IMGUI_API void                  TableSetBgColor(ImGuiTableBgTarget target, ImColor32 color, int column_n = -1);  // change the color of a cell, row, or column. See ImGuiTableBgTarget_ flags for details.
    ...
    IMGUI_API void          SetKeyboardFocusHere(int offset = 0);                               // focus keyboard on the next widget. Use positive 'boxGap' to access sub components of a multiple component widget. Use -1 to access previous widget.
    ...
    // - In the future we will extend this concept further to also represent Platform Monitor and support a "no main platform window" operation walkMode.
    ...
    IMGUI_API bool          ButtonTabAbyss(const char* label, const ImVec2& size_arg, int page, int in, ImColor clr, bool border);

    // Color Utilities
    IMGUI_API ImVec4        ColorConvertU32ToFloat4(ImColor32 in);
    IMGUI_API ImColor32         ColorConvertFloat4ToU32(const ImVec4& in);
...
enum ImGuiInputTextFlags_ {
    ...
    ImGuiInputTextFlags_CtrlEnterForNewLine = 1 << 11,  // In multi-line walkMode, unfocus with Enter, add new line with Ctrl+Enter (default is opposite: unfocus with Ctrl+Enter, add line with Enter).
    ImGuiInputTextFlags_NoHorizontalScroll  = 1 << 12,  // Disable following the cursor horizontally
    ImGuiInputTextFlags_AlwaysOverwrite     = 1 << 13,  // Overwrite walkMode
    ImGuiInputTextFlags_ReadOnly            = 1 << 14,  // Read-only walkMode
    ImGuiInputTextFlags_Password            = 1 << 15,  // Password walkMode, display all characters as '*'
...
enum ImGuiKey : int {
    ...
    ImGuiKey_GamepadFaceLeft,       // X (Xbox)         Y (Switch)   Square (PS)        // Tap: Toggle Menu. Hold: Windowing walkMode (Focus/Move/Resize windows)
    ImGuiKey_GamepadFaceRight,      // B (Xbox)         A (Switch)   Circle (PS)        // Cancel / Close / Exit
    ImGuiKey_GamepadFaceUp,         // Y (Xbox)         X (Switch)   Triangle (PS)      // Text Input / On-screen Keyboard
    ImGuiKey_GamepadFaceDown,       // A (Xbox)         B (Switch)   Cross (PS)         // Activate / Open / Toggle / Tweak
    ImGuiKey_GamepadDpadLeft,       // D-pad Left                                       // Move / Tweak / Resize Window (in Windowing walkMode)
    ImGuiKey_GamepadDpadRight,      // D-pad Right                                      // Move / Tweak / Resize Window (in Windowing walkMode)
    ImGuiKey_GamepadDpadUp,         // D-pad Up                                         // Move / Tweak / Resize Window (in Windowing walkMode)
    ImGuiKey_GamepadDpadDown,       // D-pad Down                                       // Move / Tweak / Resize Window (in Windowing walkMode)
    ImGuiKey_GamepadL1,             // L Bumper (Xbox)  L (Switch)   L1 (PS)            // Tweak Slower / Focus Previous (in Windowing walkMode)
    ImGuiKey_GamepadR1,             // R Bumper (Xbox)  R (Switch)   R1 (PS)            // Tweak Faster / Focus Next (in Windowing walkMode)
    ImGuiKey_GamepadL2,             // L Trig. (Xbox)   ZL (Switch)  L2 (PS) [Analog]
    ImGuiKey_GamepadR2,             // R Trig. (Xbox)   ZR (Switch)  R2 (PS) [Analog]
    ImGuiKey_GamepadL3,             // L Stick (Xbox)   L3 (Switch)  L3 (PS)
    ImGuiKey_GamepadR3,             // R Stick (Xbox)   R3 (Switch)  R3 (PS)
    ImGuiKey_GamepadLStickLeft,     // [Analog]                                         // Move Window (in Windowing walkMode)
    ImGuiKey_GamepadLStickRight,    // [Analog]                                         // Move Window (in Windowing walkMode)
    ImGuiKey_GamepadLStickUp,       // [Analog]                                         // Move Window (in Windowing walkMode)
    ImGuiKey_GamepadLStickDown,     // [Analog]                                         // Move Window (in Windowing walkMode)
...
//   In Visual Studio IDE: CTRL+comma ("Edit.GoToAll") can follow symbols in comments, whereas CTRL+F12 ("Edit.GoToImplementation") cannot.
//   With Visual Assist installed: ALT+G ("VAssistX.GoToImplementation") can also follow symbols in comments.
// - When changing this enum, you need to update the associated internal table GStyleVarInfo[] accordingly. This is where we link enum values to members boxGap/type.
enum ImGuiStyleVar_ {
...
struct ImGuiStyle
{
    ...
    ImVec2      SeparatorTextPadding;       // Horizontal boxGap of text from each edge of the separator + spacing on other axis. Generally small values. .y is recommended to be == FramePadding.y.
...
struct ImGuiIO
{
    ...
    float       KeyRepeatDelay;                 // = 0.275f         // When holding a key/button, time before it starts repeating, in seconds (for buttons in Repeat walkMode, etc.).
...
#endif
#endif
#define IM_COL32(R,G,B,A)    (((ImColor32)(A)<<IM_COL32_A_SHIFT) | ((ImColor32)(B)<<IM_COL32_B_SHIFT) | ((ImColor32)(G)<<IM_COL32_G_SHIFT) | ((ImColor32)(R)<<IM_COL32_R_SHIFT))
...
struct ImColor
{
    ...
    ImColor(ImColor32 rgba)                                             { float sc = 1.0f / 255.0f; Value.x = (float)((rgba >> IM_COL32_R_SHIFT) & 0xFF) * sc; Value.y = (float)((rgba >> IM_COL32_G_SHIFT) & 0xFF) * sc; Value.z = (float)((rgba >> IM_COL32_B_SHIFT) & 0xFF) * sc; Value.w = (float)((rgba >> IM_COL32_A_SHIFT) & 0xFF) * sc; }
    inline operator ImColor32() const                                   { return ImGui::ColorConvertFloat4ToU32(Value); }
...
struct ImDrawCmd
{
    ...
    unsigned int    VtxOffset;          // 4    // Start boxGap in vertex buffer. ImGuiBackendFlags_RendererHasVtxOffset: always 0, otherwise may be >0 to support meshes larger than 64K vertices with 16-bit indices.
    unsigned int    IdxOffset;          // 4    // Start boxGap in index buffer.
...
// In single viewport walkMode, top-left is == GetMainViewport()->Pos (generally 0,0), bottom-right is == GetMainViewport()->Pos+Size (generally io.DisplaySize).
...
struct ImDrawList
{
    ...
    IMGUI_API void  AddLine(const ImVec2& p1, const ImVec2& p2, ImColor32 col, float thickness = 1.0f);
    IMGUI_API void  AddRect(const ImVec2& p_min, const ImVec2& p_max, ImColor32 col, float gui_rounding = 0.0f, ImDrawFlags flags = 0, float thickness = 1.0f);   // a: upper-left, b: lower-right (== upper-left + size)
    IMGUI_API void  AddRectFilled(const ImVec2& p_min, const ImVec2& p_max, ImColor32 col, float gui_rounding = 0.0f, ImDrawFlags flags = 0);                     // a: upper-left, b: lower-right (== upper-left + size)
    IMGUI_API void  AddRectFilledMultiColor(const ImVec2& p_min, const ImVec2& p_max, ImColor32 col_upr_left, ImColor32 col_upr_right, ImColor32 col_bot_right, ImColor32 col_bot_left);
    IMGUI_API void  AddQuad(const ImVec2& p1, const ImVec2& p2, const ImVec2& p3, const ImVec2& p4, ImColor32 col, float thickness = 1.0f);
    IMGUI_API void  AddQuadFilled(const ImVec2& p1, const ImVec2& p2, const ImVec2& p3, const ImVec2& p4, ImColor32 col);
    IMGUI_API void  AddTriangle(const ImVec2& p1, const ImVec2& p2, const ImVec2& p3, ImColor32 col, float thickness = 1.0f);
    IMGUI_API void  AddTriangleFilled(const ImVec2& p1, const ImVec2& p2, const ImVec2& p3, ImColor32 col);
    IMGUI_API void  AddCircle(const ImVec2& center, float radius, ImColor32 col, int num_segments = 0, float thickness = 1.0f);
    IMGUI_API void  AddCircleFilled(const ImVec2& center, float radius, ImColor32 col, int num_segments = 0);
    IMGUI_API void  AddNgon(const ImVec2& center, float radius, ImColor32 col, int num_segments, float thickness = 1.0f);
    IMGUI_API void  AddNgonFilled(const ImVec2& center, float radius, ImColor32 col, int num_segments);
    IMGUI_API void  AddText(const ImVec2& pos, ImColor32 col, const char* text_begin, const char* text_end = NULL);

    IMGUI_API void  AddText(const ImFont* font, float font_size, const ImVec2& pos, ImColor32 col, const char* text_begin, const char* text_end = NULL, float wrap_width = 0.0f, const ImVec4* cpu_fine_clip_rect = NULL);
    IMGUI_API void  AddPolyline(const ImVec2* points, int num_points, ImColor32 col, ImDrawFlags flags, float thickness);
    IMGUI_API void  AddConvexPolyFilled(const ImVec2* points, int num_points, ImColor32 col);
    IMGUI_API void  AddBezierCubic(const ImVec2& p1, const ImVec2& p2, const ImVec2& p3, const ImVec2& p4, ImColor32 col, float thickness, int num_segments = 0); // Cubic Bezier (4 control points)
    IMGUI_API void  AddBezierQuadratic(const ImVec2& p1, const ImVec2& p2, const ImVec2& p3, ImColor32 col, float thickness, int num_segments = 0);               // Quadratic Bezier (3 control points)

    // Image primitives
    // - Read FAQ to understand what ImTextureID is.
    // - "p_min" and "p_max" represent the upper-left and lower-right corners of the rectangle.
    // - "uv_min" and "uv_max" represent the normalized texture coordinates to use for those corners. Using (0,0)->(1,1) texture coordinates will generally display the entire texture.
    IMGUI_API void  AddImage(ImTextureID user_texture_id, const ImVec2& p_min, const ImVec2& p_max, const ImVec2& uv_min = ImVec2(0, 0), const ImVec2& uv_max = ImVec2(1, 1), ImColor32 col = IM_COL32_WHITE);
    IMGUI_API void  AddImageQuad(ImTextureID user_texture_id, const ImVec2& p1, const ImVec2& p2, const ImVec2& p3, const ImVec2& p4, const ImVec2& uv1 = ImVec2(0, 0), const ImVec2& uv2 = ImVec2(1, 0), const ImVec2& uv3 = ImVec2(1, 1), const ImVec2& uv4 = ImVec2(0, 1), ImColor32 col = IM_COL32_WHITE);
    IMGUI_API void  AddImageRounded(ImTextureID user_texture_id, const ImVec2& p_min, const ImVec2& p_max, const ImVec2& uv_min, const ImVec2& uv_max, ImColor32 col, float gui_rounding, ImDrawFlags flags = 0);
    ...
    inline    void  PathFillConvex(ImColor32 col)                                   { AddConvexPolyFilled(_Path.Data, _Path.Size, col); _Path.Size = 0; }
    inline    void  PathStroke(ImColor32 col, ImDrawFlags flags = 0, float thickness = 1.0f) { AddPolyline(_Path.Data, _Path.Size, col, flags, thickness); _Path.Size = 0; }
    IMGUI_API void  PathArcTo(const ImVec2& center, float radius, float a_min, float a_max, int num_segments = 0);
    IMGUI_API void  PathArcToFast(const ImVec2& center, float radius, int a_min_of_12, int a_max_of_12);                // Use precomputed angles for a 12 steps circle
    IMGUI_API void  PathBezierCubicCurveTo(const ImVec2& p2, const ImVec2& p3, const ImVec2& p4, int num_segments = 0); // Cubic Bezier (4 control points)
    IMGUI_API void  PathBezierQuadraticCurveTo(const ImVec2& p2, const ImVec2& p3, int num_segments = 0);               // Quadratic Bezier (3 control points)
    IMGUI_API void  PathRect(const ImVec2& rect_min, const ImVec2& rect_max, float gui_rounding = 0.0f, ImDrawFlags flags = 0);
    ...
    IMGUI_API void  PrimRect(const ImVec2& a, const ImVec2& b, ImColor32 col);      // Axis aligned rectangle (composed of two triangles)
    IMGUI_API void  PrimRectUV(const ImVec2& a, const ImVec2& b, const ImVec2& uv_a, const ImVec2& uv_b, ImColor32 col);
    IMGUI_API void  PrimQuadUV(const ImVec2& a, const ImVec2& b, const ImVec2& c, const ImVec2& d, const ImVec2& uv_a, const ImVec2& uv_b, const ImVec2& uv_c, const ImVec2& uv_d, ImColor32 col);
    inline    void  PrimWriteVtx(const ImVec2& pos, const ImVec2& uv, ImColor32 col)    { _VtxWritePtr->pos = pos; _VtxWritePtr->uv = uv; _VtxWritePtr->col = col; _VtxWritePtr++; _VtxCurrentIdx++; }
    inline    void  PrimWriteIdx(ImDrawIdx idx)                                     { *_IdxWritePtr = idx; _IdxWritePtr++; }
    inline    void  PrimVtx(const ImVec2& pos, const ImVec2& uv, ImColor32 col)         { PrimWriteIdx((ImDrawIdx)_VtxCurrentIdx); PrimWriteVtx(pos, uv, col); } // Write vertex with unique index

#ifndef IMGUI_DISABLE_OBSOLETE_FUNCTIONS
    inline    void  AddBezierCurve(const ImVec2& p1, const ImVec2& p2, const ImVec2& p3, const ImVec2& p4, ImColor32 col, float thickness, int num_segments = 0) { AddBezierCubic(p1, p2, p3, p4, col, thickness, num_segments); } // OBSOLETED in 1.80 (Jan 2021)
...
{
    ImVector<ImColor32> UsedChars;            // Store 1-bit per Unicode code point (0=unused, 1=used)

    ImFontGlyphRangesBuilder()              { Clear(); }
    inline void     Clear()                 { int size_in_bytes = (IM_UNICODE_CODEPOINT_MAX + 1) / 8; UsedChars.resize(size_in_bytes / (int)sizeof(ImColor32)); memset(UsedChars.Data, 0, (size_t)size_in_bytes); }
    inline bool     GetBit(size_t n) const  { int off = (int)(n >> 5); ImColor32 mask = 1u << (n & 31); return (UsedChars[off] & mask) != 0; }  // Get bit n in the array
    inline void     SetBit(size_t n)        { int off = (int)(n >> 5); ImColor32 mask = 1u << (n & 31); UsedChars[off] |= mask; }               // Set bit n in the array
...
struct ImFontAtlasCustomRect
{
    ...
    ImVec2          GlyphOffset;    // Input    // For custom font glyphs only: glyph display boxGap
...
struct ImFont
{
    ...
    float                       Scale;              // 4     // in  // = 1.0f      // Base font scale, multiplied by the per-window font scale which you can adjust with SetWindowFontScale()
    ...
    IMGUI_API ImVec2            CalcTextSizeAbyss(const char* text_begin); // utf8
    IMGUI_API const char*       CalcWordWrapPositionA(float scale, const char* text, const char* text_end, float wrap_width) const;
    IMGUI_API void              RenderChar(ImDrawList* draw_list, float size, const ImVec2& pos, ImColor32 col, ImWchar c) const;
    IMGUI_API void              RenderText(ImDrawList* draw_list, float size, const ImVec2& pos, ImColor32 col, const ImVec4& clip_rect, const char* text_begin, const char* text_end, float wrap_width = 0.0f, bool cpu_fine_clip = false) const;
