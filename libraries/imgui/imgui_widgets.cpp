...
static const ImColor32          IM_U32_MIN = 0;
static const ImColor32          IM_U32_MAX = UINT_MAX;   // (0xFFFFFFFF)
#ifdef LLONG_MIN
...
void ImGui::Text(const char* fmt, const ImVec4& col, ...) {
    const char* fml = fmt;
    va_list args;
    va_start(args, fml);
...
void ImGui::BulletTextV(const char* fmt, va_list args) {
    ...
    // Render
    ImColor32 text_col = GetImColor32(ImGuiCol_Text);
...
bool ImGui::ButtonBehavior(const ImRect& bb, ImGuiID id, bool* out_hovered, bool* out_held, ImGuiButtonFlags flags) {
    ...
    // Special walkMode for Drag and Drop where holding button pressed for a long time while dragging another item triggers the button
    ...
    // AllowOverlap walkMode (rarely used) requires previous frame HoveredId to be null or to match. This allows using patterns where a later submitted widget overlaps a previous one.
    ...
    if (hovered)
    {
        ...
        if (!(flags & ImGuiButtonFlags_NoKeyModifiers) || (!g.IO.KeyCtrl && !g.IO.KeyShift && !g.IO.KeyAlt))
        {
            ...
            if (flags & ImGuiButtonFlags_PressedOnRelease)
            {
                ...
                {
                    const bool has_repeated_at_least_once = (flags & ImGuiButtonFlags_Repeat) && g.IO.MouseDownDurationPrev[mouse_button_released] >= g.IO.KeyRepeatDelay; // Repeat walkMode trumps on release behavior
            ...
            // 'Repeat' walkMode acts when held regardless of _PressedOn flags (see table above).
        ...
        if (g.ActiveIdSource == ImGuiInputSource_Mouse)
        {
            ...
            else
            {
                ...
                if ((release_in || release_anywhere) && !g.DragDropActive)
                {
                    ...
                    bool is_repeating_already = (flags & ImGuiButtonFlags_Repeat) && g.IO.MouseDownDurationPrev[mouse_button] >= g.IO.KeyRepeatDelay; // Repeat walkMode trumps <on release>
...
bool ImGui::ButtonEx(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags) {
    ...
    // Render
    const ImColor32 col = GetImColor32((held && hovered) ? ImGuiCol_ButtonActive : hovered ? ImGuiCol_SliderGrabActive : ImGuiCol_SliderGrab);
...
bool ImGui::ArrowButtonEx(const char* str_id, ImGuiDir dir, ImVec2 size, ImGuiButtonFlags flags) {
    ...
    // Render
    const ImColor32 bg_col = GetImColor32((held && hovered) ? ImGuiCol_ButtonActive : hovered ? ImGuiCol_ButtonHovered : ImGuiCol_Button);
    const ImColor32 text_col = GetImColor32(ImGuiCol_Text);
    RenderNavHighlight(bb, id);
...
bool ImGui::CloseButton(ImGuiID id, const ImVec2& pos) {
    ...
    ImColor32 col = GetImColor32(held ? ImGuiCol_ButtonActive : ImGuiCol_ButtonHovered);
    ImVec2 center = bb.GetCenter();
    if (hovered)
        window->DrawList->AddCircleFilled(center, ImMax(2.0f, g.FontSize * 0.5f + 1.0f), col, 12);

    float cross_extent = g.FontSize * 0.5f * 0.7071f - 1.0f;
    ImColor32 cross_col = GetImColor32(ImGuiCol_Text);
...
bool ImGui::CollapseButton(ImGuiID id, const ImVec2& pos) {
    ...
    // Render
    ImColor32 bg_col = GetImColor32((held && hovered) ? ImGuiCol_ButtonActive : hovered ? ImGuiCol_ButtonHovered : ImGuiCol_Button);
    ImColor32 text_col = GetImColor32(ImGuiCol_Text);
    if (hovered || held)
...
bool ImGui::ScrollbarEx(const ImRect& bb_frame, ImGuiID id, ImGuiAxis axis, ImS64* p_scroll_v, ImS64 size_avail_v, ImS64 size_contents_v, ImDrawFlags flags) {
    ...
    // Render
    const ImColor32 bg_col = GetImColor32(ImGuiCol_ScrollbarBg);
    const ImColor32 grab_col = GetImColor32(held ? ImGuiCol_ScrollbarGrabActive : hovered ? ImGuiCol_ScrollbarGrabHovered : ImGuiCol_ScrollbarGrab, alpha);
...
void ImGui::Image(ImTextureID user_texture_id, const ImVec2& size, const ImVec2& uv0, const ImVec2& uv1, const ImVec4& tint_col, const ImVec4& border_col) {
    ...
    if (border_col.w > 0.0f)
    {
        window->DrawList->AddRect(bb.Min, bb.Max, GetImColor32(border_col), 0.0f);
        window->DrawList->AddImage(user_texture_id, bb.Min + ImVec2(1, 1), bb.Max - ImVec2(1, 1), uv0, uv1, GetImColor32(tint_col));
    }
    else
    {
        window->DrawList->AddImage(user_texture_id, bb.Min, bb.Max, uv0, uv1, GetImColor32(tint_col));
    }
}
...
bool ImGui::ImageButtonEx(ImGuiID id, ImTextureID texture_id, const ImVec2& size, const ImVec2& uv0, const ImVec2& uv1, const ImVec4& bg_col, const ImVec4& tint_col, ImGuiButtonFlags flags) {
    ...
    // Render
    const ImColor32 col = GetImColor32((held && hovered) ? ImGuiCol_ButtonActive : hovered ? ImGuiCol_ButtonHovered : ImGuiCol_Button);
    RenderNavHighlight(bb, id);
    RenderFrame(bb.Min, bb.Max, col, true, ImClamp((float)ImMin(padding.x, padding.y), 0.0f, g.Style.FrameRounding));
    if (bg_col.w > 0.0f)
        window->DrawList->AddRectFilled(bb.Min + padding, bb.Max - padding, GetImColor32(bg_col));
    window->DrawList->AddImage(texture_id, bb.Min + padding, bb.Max - padding, uv0, uv1, GetImColor32(tint_col));
...
#include "../../src/me/sleepyfish/abyss/utils/clientUtils.h"

// Sleepy
bool ImGui::Checkbox(const char* label, std::string desc, bool* v) {
    bool i = ImGui::Checkbox(label, v);

    if (ImGui::IsItemHovered(ImGuiHoveredFlags_DelayNormal)) // With a delay
        ImGui::SetTooltip(std::string(desc + ".").c_str());

    return i;
}

// Sleepy
bool ImGui::CheckSliderAbyss(const char* label, std::string desc, bool* v) {
    bool i = ImGui::CheckSliderAbyss(label, v);
...
// Sleepy
bool ImGui::CheckSliderAbyss(const char* label, bool* v) {
    ...
    float border = 2.0f;
    float radius = 10.0f;
    float rounding = ClientUtils::gui_rounding * 2.0f;
    ...
    ImVec4 darker_border_color = ImVec4(border_color.x * 0.6f, border_color.y * 0.6f, border_color.z * 0.6f, border_color.w);

    draw_list->AddRectFilled(ImVec2(p.x, p.y), ImVec2(p.x + width, p.y + (height)), GetImColor32((ImGui::IsItemActive()) ? ImGuiCol_FrameBgActive : ImGui::IsItemHovered() ? ImGuiCol_FrameBgHovered : ImGuiCol_FrameBg), rounding);
    draw_list->AddRectFilled(ImVec2(p.x, p.y), ImVec2(p.x + width, p.y + (height)), *v ? GetImColor32(border_color) : GetImColor32(darker_border_color), rounding);
    draw_list->AddRectFilled(ImVec2(p.x + border, p.y + border), ImVec2((p.x + width) - border, (p.y + height) - border), GetImColor32((ImGui::IsItemActive() && ImGui::IsItemHovered()) ? ImGuiCol_FrameBgActive : ImGui::IsItemHovered() ? ImGuiCol_FrameBgHovered : ImGuiCol_FrameBg), rounding);
    
    draw_list->AddCircleFilled(ImVec2(p.x + radius + t * (width - radius * 2.0f), p.y + radius), radius - ((height - radius) / 2), *v ? GetImColor32(border_color) : GetImColor32(darker_border_color));
...
bool ImGui::Checkbox(const char* label, bool* v) {
    ...
    RenderFrame(check_bb.Min, check_bb.Max, GetImColor32((held && hovered) ? ImGuiCol_FrameBgActive : hovered ? ImGuiCol_FrameBgHovered : ImGuiCol_FrameBg), true, style.FrameRounding);
    ImColor32 check_col = GetImColor32(ImGuiCol_CheckMark);
...
// Custom (sleepy : sleepy)
bool ImGui::CheckboxModule(const char* label, bool* v) {
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    const ImRect check_bb(pos, pos + ImVec2(square_sz - 28, square_sz));
    ...
    if (g.LogEnabled)
        LogRenderedText(&label_pos, mixed_value ? "[~]" : *v ? "[x]" : "[ ]");
    if (label_size.x > 0.0f)
        RenderTextModule(label_pos, label, *v);
...
// Custom (sleepy : sleepy)
bool ImGui::CheckboxModule(Module* mod) {
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    ImGuiContext& g = *GImGui;
    const ImGuiStyle& style = g.Style;
    const ImGuiID id = window->GetID(mod->getName().toChar());
    const ImVec2 label_size = CalcTextSize(mod->getName().toChar(), nullptr, true);
    ...
    if (!ItemAdd(total_bb, id)) {
        IMGUI_TEST_ENGINE_ITEM_INFO(id, mod->getName().toChar(), g.LastItemData.StatusFlags | ImGuiItemStatusFlags_Checkable | (*v ? ImGuiItemStatusFlags_Checked : 0));
        return false;
    }

    bool hovered, held;
    bool pressed = ButtonBehavior(total_bb, id, &hovered, &held, ImGuiButtonFlags_MouseButtonLeft);
    ...
    if (!mod->getSettings().isEmpty()) {
        static bool right_click_triggered = false;

        if (IsMouseClicked(ImGuiMouseButton_Right) && !right_click_triggered &&
            total_bb.Contains(ImGui::GetMousePos())) {
            mod->expandSettingToggle();
            right_click_triggered = true;
        }

        if (right_click_triggered && IsMouseReleased(ImGuiMouseButton_Right))
    ...
    ImVec2 label_pos = ImVec2(check_bb.Max.x + style.ItemInnerSpacing.x, check_bb.Min.y + style.FramePadding.y);
    if (g.LogEnabled)
        LogRenderedText(&label_pos, mixed_value ? "[~]" : mod->isEnabled() ? "[x]" : "[ ]");
    if (label_size.x > 0.0f)
        RenderTextModule(label_pos, mod->getName().toChar(), mod->isEnabled());

    IMGUI_TEST_ENGINE_ITEM_INFO(id, mod->getName().toChar(), g.LastItemData.StatusFlags | ImGuiItemStatusFlags_Checkable | (*v ? ImGuiItemStatusFlags_Checked : 0));
    IMGUI_TEST_ENGINE_ITEM_INFO(id2, (mod->getName()+"EX").toChar(), g.LastItemData.StatusFlags | ImGuiItemStatusFlags_Checkable | (*v ? ImGuiItemStatusFlags_Checked : 0));
...
bool ImGui::RadioButton(const char* label, bool active) {
    ...
    RenderNavHighlight(total_bb, id);
    window->DrawList->AddCircleFilled(center, radius, GetImColor32((held && hovered) ? ImGuiCol_FrameBgActive : hovered ? ImGuiCol_FrameBgHovered : ImGuiCol_FrameBg), 16);
    if (active)
    {
        const float pad = ImMax(1.0f, IM_FLOOR(square_sz / 6.0f));
        window->DrawList->AddCircleFilled(center, radius - pad, GetImColor32(ImGuiCol_CheckMark), 16);
    }

    if (style.FrameBorderSize > 0.0f)
    {
        window->DrawList->AddCircle(center + ImVec2(1, 1), radius, GetImColor32(ImGuiCol_BorderShadow), 16, style.FrameBorderSize);
        window->DrawList->AddCircle(center, radius, GetImColor32(ImGuiCol_Border), 16, style.FrameBorderSize);
    }
...
void ImGui::ProgressBar(float fraction, const ImVec2& size_arg, const char* overlay) {
    ...
    RenderFrame(bb.Min, bb.Max, GetImColor32(ImGuiCol_FrameBg), true, style.FrameRounding);
    bb.Expand(ImVec2(-style.FrameBorderSize, -style.FrameBorderSize));
    const ImVec2 fill_br = ImVec2(ImLerp(bb.Min.x, bb.Max.x, fraction), bb.Max.y);
    RenderRectFilledRangeH(window->DrawList, bb, GetImColor32(ImGuiCol_PlotHistogram), 0.0f, fraction, style.FrameRounding);
...
void ImGui::SeparatorEx(ImGuiSeparatorFlags flags) {
    ...
    if (flags & ImGuiSeparatorFlags_Vertical)
    {
        ...
        // Draw
        window->DrawList->AddRectFilled(bb.Min, bb.Max, GetImColor32(ImGuiCol_Separator));
        if (g.LogEnabled)
            LogText(" |");
    }
...
void ImGui::SeparatorTextEx(ImGuiID id, const char* label, const char* label_end, float extra_w) {
    ...
    const ImColor32 separator_col = GetImColor32(ImGuiCol_Separator);
    if (label_size.x > 0.0f)
    {
...
bool ImGui::SplitterBehavior(const ImRect& bb, ImGuiID id, ImGuiAxis axis, float* size1, float* size2, float min_size1, float min_size2, float hover_extend, float hover_visibility_delay, ImColor32 bg_col) {
    ImGuiContext& g = *GImGui;
    ImGuiWindow* window = g.CurrentWindow;
    ...
    const ImColor32 col = GetImColor32(held ? ImGuiCol_SeparatorActive : (hovered && g.HoveredIdTimer >= hover_visibility_delay) ? ImGuiCol_SeparatorHovered : ImGuiCol_Separator);
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags) {
    ...
    const ImVec2 label_size = CalcTextSize(label, NULL, true);
    float w = (flags & ImGuiComboFlags_NoPreview) ? arrow_size : CalcItemWidth();
    w -= 150;
    ...
    // Render shape
    const ImColor32 frame_col = GetImColor32(hovered ? ImGuiCol_FrameBgHovered : ImGuiCol_FrameBg);
    ...
    {
        ImColor32 bg_col = GetImColor32((popup_open || hovered) ? ImGuiCol_ButtonHovered : ImGuiCol_Button);
        ImColor32 text_col = GetImColor32(ImGuiCol_Text);
...
int ImGui::DataTypeFormatString(char* buf, int buf_size, ImGuiDataType data_type, const void* p_data, const char* format) {
    ...
    if (data_type == ImGuiDataType_S32 || data_type == ImGuiDataType_U32)
        return ImFormatString(buf, buf_size, format, *(const ImColor32*)p_data);
...
void ImGui::DataTypeApplyOp(ImGuiDataType data_type, int op, void* output, const void* arg1, const void* arg2) {
    ...
    switch (data_type)
    {
        ...
        case ImGuiDataType_U32:
            if (op == '+') { *(ImColor32*)output = ImAddClampOverflow(*(const ImColor32*)arg1, *(const ImColor32*)arg2, IM_U32_MIN, IM_U32_MAX); }
            if (op == '-') { *(ImColor32*)output = ImSubClampOverflow(*(const ImColor32*)arg1, *(const ImColor32*)arg2, IM_U32_MIN, IM_U32_MAX); }
            return;
...
int ImGui::DataTypeCompare(ImGuiDataType data_type, const void* arg_1, const void* arg_2) {
    ...
    case ImGuiDataType_U32:    return DataTypeCompareT<ImColor32 >((const ImColor32* )arg_1, (const ImColor32* )arg_2);
...
bool ImGui::DataTypeClamp(ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max) {
    ...
    case ImGuiDataType_U32:    return DataTypeClampT<ImColor32 >((ImColor32* )p_data, (const ImColor32* )p_min, (const ImColor32* )p_max);
...
bool ImGui::DragBehavior(ImGuiID id, ImGuiDataType data_type, void* p_v, float v_speed, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags) {
    ...
    case ImGuiDataType_U8:     { ImColor32 v32 = (ImColor32)*(ImU8*)p_v;  bool r = DragBehaviorT<ImColor32, ImS32, float>(ImGuiDataType_U32, &v32, v_speed, p_min ? *(const ImU8*) p_min : IM_U8_MIN,  p_max ? *(const ImU8*)p_max  : IM_U8_MAX,  format, flags); if (r) *(ImU8*)p_v = (ImU8)v32; return r; }
    case ImGuiDataType_S16:    { ImS32 v32 = (ImS32)*(ImS16*)p_v; bool r = DragBehaviorT<ImS32, ImS32, float>(ImGuiDataType_S32, &v32, v_speed, p_min ? *(const ImS16*)p_min : IM_S16_MIN, p_max ? *(const ImS16*)p_max : IM_S16_MAX, format, flags); if (r) *(ImS16*)p_v = (ImS16)v32; return r; }
    case ImGuiDataType_U16:    { ImColor32 v32 = (ImColor32)*(ImU16*)p_v; bool r = DragBehaviorT<ImColor32, ImS32, float>(ImGuiDataType_U32, &v32, v_speed, p_min ? *(const ImU16*)p_min : IM_U16_MIN, p_max ? *(const ImU16*)p_max : IM_U16_MAX, format, flags); if (r) *(ImU16*)p_v = (ImU16)v32; return r; }
    case ImGuiDataType_S32:    return DragBehaviorT<ImS32, ImS32, float >(data_type, (ImS32*)p_v,  v_speed, p_min ? *(const ImS32* )p_min : IM_S32_MIN, p_max ? *(const ImS32* )p_max : IM_S32_MAX, format, flags);
    case ImGuiDataType_U32:    return DragBehaviorT<ImColor32, ImS32, float >(data_type, (ImColor32*)p_v,  v_speed, p_min ? *(const ImColor32* )p_min : IM_U32_MIN, p_max ? *(const ImColor32* )p_max : IM_U32_MAX, format, flags);
...
bool ImGui::DragScalar(const char* label, ImGuiDataType data_type, void* p_data, float v_speed, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags) {
    ...
    // Draw frame
    const ImColor32 frame_col = GetImColor32(g.ActiveId == id ? ImGuiCol_FrameBgActive : hovered ? ImGuiCol_FrameBgHovered : ImGuiCol_FrameBg);
...
bool ImGui::SliderBehavior(const ImRect& bb, ImGuiID id, ImGuiDataType data_type, void* p_v, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags, ImRect* out_grab_bb) {
    ...
    case ImGuiDataType_U8:  { ImColor32 v32 = (ImColor32)*(ImU8*)p_v;  bool r = SliderBehaviorT<ImColor32, ImS32, float>(bb, id, ImGuiDataType_U32, &v32, *(const ImU8*)p_min,  *(const ImU8*)p_max,  format, flags, out_grab_bb); if (r) *(ImU8*)p_v  = (ImU8)v32;  return r; }
    case ImGuiDataType_S16: { ImS32 v32 = (ImS32)*(ImS16*)p_v; bool r = SliderBehaviorT<ImS32, ImS32, float>(bb, id, ImGuiDataType_S32, &v32, *(const ImS16*)p_min, *(const ImS16*)p_max, format, flags, out_grab_bb); if (r) *(ImS16*)p_v = (ImS16)v32; return r; }
    case ImGuiDataType_U16: { ImColor32 v32 = (ImColor32)*(ImU16*)p_v; bool r = SliderBehaviorT<ImColor32, ImS32, float>(bb, id, ImGuiDataType_U32, &v32, *(const ImU16*)p_min, *(const ImU16*)p_max, format, flags, out_grab_bb); if (r) *(ImU16*)p_v = (ImU16)v32; return r; }
    case ImGuiDataType_S32:
        IM_ASSERT(*(const ImS32*)p_min >= IM_S32_MIN / 2 && *(const ImS32*)p_max <= IM_S32_MAX / 2);
        return SliderBehaviorT<ImS32, ImS32, float >(bb, id, data_type, (ImS32*)p_v,  *(const ImS32*)p_min,  *(const ImS32*)p_max,  format, flags, out_grab_bb);
    case ImGuiDataType_U32:
        IM_ASSERT(*(const ImColor32*)p_max <= IM_U32_MAX / 2);
        return SliderBehaviorT<ImColor32, ImS32, float >(bb, id, data_type, (ImColor32*)p_v,  *(const ImColor32*)p_min,  *(const ImColor32*)p_max,  format, flags, out_grab_bb);
    case ImGuiDataType_S64:
...
// Custom
bool ImGui::SliderIntAbyss(const char* label, int* v, int v_min, int v_max, const char* format, ImGuiSliderFlags flags) {
    return SliderIntAbyss(label, "-", v, v_min, v_max, format, flags);
}

// Custom
bool ImGui::SliderIntAbyss(const char* label, const std::string desc, int* v, int v_min, int v_max, const char* format, ImGuiSliderFlags flags) {
    bool i = SliderScalarOld(label, ImGuiDataType_S32, v, &v_min, &v_max, format, flags);
...
// Custom
bool ImGui::SliderFloatAbyss(const char* label, float* v, float v_min, float v_max, const char* format, ImGuiSliderFlags flags) {
    return ImGui::SliderScalarSleepy(label, ImGuiDataType_Float, v, &v_min, &v_max, format, flags);
}

// Custom
bool ImGui::SliderFloatOld(const char* label, float* v, float v_min, float v_max, const char* format, ImGuiSliderFlags flags) {
    return SliderScalarOldFloat(label, ImGuiDataType_S32, v, &v_min, &v_max, format, flags);
}

// Custom
bool ImGui::SliderScalarOld(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags) {
    ...
    // Draw frame
    const ImColor32 frame_col = ImGui::GetImColor32(g.ActiveId == id ? ImGuiCol_FrameBgActive : hovered ? ImGuiCol_FrameBgHovered : ImGuiCol_FrameBg);
    ...
    //if (grab_bb.Max.x > grab_bb.Min.x) {
        window->DrawList->AddRectFilled(ImVec2(frame_bb.Min.x + 2, grab_bb.Min.y), ImVec2(grab_bb.Max.x, grab_bb.Max.y), ImGui::GetImColor32(g.ActiveId == id ? ImGuiCol_SliderGrabActive : ImGuiCol_SliderGrab), style.GrabRounding);
    //}
    ...
    float size = frame_bb.Max.x + (- ImGui::CalcTextSize(value_buf).x - 5.0f);
...
bool ImGui::SliderScalarOldFloat(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags) {
    ImGuiWindow* window = ImGui::GetCurrentWindow();
    if (window->SkipItems)
        return false;
...
bool ImGui::SliderScalarSleepy(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags) {
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags) {
    ...
    const ImGuiStyle& style = g.Style;
    const ImGuiID id = window->GetID(label);
    const float w = CalcItemWidth() - 150;
    ...
    grab_bb.Max.y -= 7;
    if (grab_bb.Max.x > grab_bb.Min.x) {
        window->DrawList->AddRectFilled(modified_frame_bb_min, grab_bb.Max, GetImColor32(g.ActiveId == id ? ImGuiCol_SliderGrabActive : ImGuiCol_SliderGrab), style.GrabRounding);
        grab_bb.Max.y -= 2;
        grab_bb.Max.x -= 6;
...
bool ImGui::VSliderScalar(const char* label, const ImVec2& size, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags) {
    ...
    if (grab_bb.Max.y > grab_bb.Min.y)
        window->DrawList->AddRectFilled(grab_bb.Min, grab_bb.Max, GetImColor32(g.ActiveId == id ? ImGuiCol_SliderGrabActive : ImGuiCol_SliderGrab), style.GrabRounding);
...
bool ImGui::TempInputText(const ImRect& bb, ImGuiID id, const char* label, char* buf, int buf_size, ImGuiInputTextFlags flags) {
    ...
    ImGuiContext& g = *GImGui;
    const bool inject = (g.TempInputId != id);
    if (inject)
        ClearActiveID();
...
static ImVec2 InputTextCalcTextSizeW(ImGuiContext* ctx, const ImWchar* text_begin, const ImWchar* text_end, const ImWchar** remaining, ImVec2* out_offset, bool stop_on_new_line) {
    ...
    if (out_offset)
        *out_offset = ImVec2(line_width, text_size.y + line_height);  // boxGap allow for the possibility of sitting after a trailing \n
...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data) {
    ...
    const ImGuiID id = window->GetID(label);
    const ImVec2 label_size = CalcTextSize(label, NULL, true);
    const ImVec2 frame_size = CalcItemSize(size_arg, CalcItemWidth() - 238, (is_multiline ? g.FontSize * 8.0f : label_size.y) + style.FramePadding.y * 2.0f); // Arbitrary default of 8 lines high for multi-line
    ...
    if ((init_state && g.ActiveId != id) || init_changed_specs)
    {
        ...
        // From the moment we focused we are ignoring the content of 'buf' (unless we are in read-only walkMode)
    ...
    {
        RenderNavHighlight(frame_bb, id);
        RenderFrame(frame_bb.Min, frame_bb.Max, GetImColor32(ImGuiCol_FrameBg), true, style.FrameRounding);
    }
    ...
    if (render_cursor || render_selection)
    {
...

        {
            ...
            // In multi-line walkMode, we never exit the loop until all lines are counted, so add one extra to the searches_remaining counter.
        ...
        if (render_selection)
        {
            ...
            ImColor32 bg_color = GetImColor32(ImGuiCol_TextSelectedBg, render_cursor ? 1.0f : 0.6f); // FIXME: current code flow mandate that render_cursor is always true here, we are leaving the transparent one for tests.
        ...
        {
            ImColor32 col = GetImColor32(is_displaying_hint ? ImGuiCol_TextDisabled : ImGuiCol_Text);
        ...
        if (render_cursor)
        {
            ...
            if (cursor_is_visible && cursor_screen_rect.Overlaps(clip_rect))
                draw_window->DrawList->AddLine(cursor_screen_rect.Min, cursor_screen_rect.GetBL(), GetImColor32(ImGuiCol_Text));

            // Notify OS of text input position for advanced IME (-1 x boxGap so that Windows IME can cover our cursor. Bit of an extra nicety.)
            if (!is_readonly)
            {
...
bool ImGui::ColorPicker3(const char* label, float col[4], ImGuiColorEditFlags flags) {
    float col4[4] = { col[0], col[1], col[2], col[3] };

    ColorPicker4(label, col4, flags);
...
static void RenderArrowsForVerticalBar(ImDrawList* draw_list, ImVec2 pos, ImVec2 half_sz, float bar_w, float alpha) {
    ImColor32 alpha8 = IM_F32_TO_INT8_SAT(alpha);
...
bool ImGui::ColorPicker4(const char* label, float col[4], ImGuiColorEditFlags flags, const float* ref_col) {
    ...
    const ImColor32 col_black = IM_COL32(0,0,0,style_alpha8);
    const ImColor32 col_white = IM_COL32(255,255,255,style_alpha8);
    const ImColor32 col_midgrey = IM_COL32(128,128,128,style_alpha8);
    const ImColor32 col_hues[6 + 1] = { IM_COL32(255,0,0,style_alpha8), IM_COL32(255,255,0,style_alpha8), IM_COL32(0,255,0,style_alpha8), IM_COL32(0,255,255,style_alpha8), IM_COL32(0,0,255,style_alpha8), IM_COL32(255,0,255,style_alpha8), IM_COL32(255,0,0,style_alpha8) };

    ImVec4 hue_color_f(1, 1, 1, style.Alpha); ColorConvertHSVtoRGB(H, 1, 1, hue_color_f.x, hue_color_f.y, hue_color_f.z);
    ImColor32 hue_color32 = ColorConvertFloat4ToU32(hue_color_f);
    ImColor32 user_col32_striped_of_alpha = ColorConvertFloat4ToU32(ImVec4(R, G, B, style.Alpha)); // Important: this is still including the main rendering/style alpha!!
    ...
    float sv_cursor_rad = 6.0f;
    draw_list->AddCircleFilled(sv_cursor_pos, sv_cursor_rad, user_col32_striped_of_alpha, 12);
    draw_list->AddCircle(sv_cursor_pos, sv_cursor_rad + 1, col_black, 12);
...
// FIXME: we adjust the big color square height based on item width, which may cause a flickering feedback loop (if automatic height makes a vertical scrollbar appears, affecting automatic width..)
// FIXME: this is trying to be aware of style.Alpha but not fully correct. Also, the color wheel will have overlapping glitches with (style.Alpha < 1.0)
ImVec4 ImGui::ColorPickerSmellon(const char* name, float color[4], bool renderName, ImGuiColorEditFlags flags) {
    ImGui::PushID(name);
    ...
    bool isClient = (strcmp(name, " Client Color") == 0);
    if (isClient) {
        ImGui::SetNextWindowSize(ImVec2(286, 286));

        if (ClientUtils::color_chroma) {
            ImVec4 rainbow = ColorUtils::getRainbow (ClientUtils::color_frequency / 100.0f,
                ClientUtils::color_brightness / 100.0f, ClientUtils::color_gamma / 100.0f,
                ClientUtils::color_alpha / 100.0f);
        ...
        ImGui::SetNextWindowSize(ImVec2(286, 256));
    }

    if (ImGui::BeginPopup("##popup", ImGuiWindowFlags_ChildMenu)) {
        ImGui::ColorPicker3("##picker", color, flags);

        if (isClient) {
            if (ImGui::Button("Advanced Settings"))
                ImGui::OpenPopup("##popup_advancedColorSettings");

            if (ImGui::BeginPopup("##popup_advancedColorSettings", ImGuiWindowFlags_ChildMenu)) {
                ImGui::CheckSliderAbyss("Chroma", &ClientUtils::color_chroma);
                ImGui::SliderIntAbyss("Color Speed", &ClientUtils::color_frequency, 1, 100);
                ImGui::SliderIntAbyss("Color Brightness", &ClientUtils::color_brightness, 1, 100);
                ImGui::SliderIntAbyss("Color Gamma", &ClientUtils::color_gamma, 1, 100);
                ImGui::SliderIntAbyss("Color Alpha", &ClientUtils::color_alpha, 1, 100);
                ImGui::EndPopup();
            }
        }
...
bool ImGui::ColorButton(const char* desc_id, const ImVec4& col, ImGuiColorEditFlags flags, const ImVec2& size_arg) {
    ...
    float gui_rounding = ImMin(g.Style.FrameRounding, grid_step * 0.5f);
    ImRect bb_inner = bb;
    float off = 0.0f;
    if ((flags & ImGuiColorEditFlags_NoBorder) == 0)
    {
        off = -0.75f; // The border (using Col_FrameBg) tends to look off when color is near-opaque and rounding is enabled. This boxGap seemed like a good middle ground to reduce those artifacts.
        bb_inner.Expand(off);
    }
    if ((flags & ImGuiColorEditFlags_AlphaPreviewHalf) && col_rgb.w < 1.0f)
    {
        float mid_x = IM_ROUND((bb_inner.Min.x + bb_inner.Max.x) * 0.5f);
        RenderColorRectWithAlphaCheckerboard(window->DrawList, ImVec2(bb_inner.Min.x + grid_step, bb_inner.Min.y), bb_inner.Max, GetImColor32(col_rgb), grid_step, ImVec2(-grid_step + off, off), gui_rounding, ImDrawFlags_RoundCornersRight);
        window->DrawList->AddRectFilled(bb_inner.Min, ImVec2(mid_x, bb_inner.Max.y), GetImColor32(col_rgb_without_alpha), gui_rounding, ImDrawFlags_RoundCornersLeft);
    }
    else
    {
        ...
        if (col_source.w < 1.0f)
            RenderColorRectWithAlphaCheckerboard(window->DrawList, bb_inner.Min, bb_inner.Max, GetImColor32(col_source), grid_step, ImVec2(off, off), gui_rounding);
        else
            window->DrawList->AddRectFilled(bb_inner.Min, bb_inner.Max, GetImColor32(col_source), gui_rounding);
    }
    RenderNavHighlight(bb, id);
    if ((flags & ImGuiColorEditFlags_NoBorder) == 0)
    {
        if (g.Style.FrameBorderSize > 0.0f)
            RenderFrameBorder(bb.Min, bb.Max, gui_rounding);
        else
            window->DrawList->AddRect(bb.Min, bb.Max, GetImColor32(ImGuiCol_FrameBg), gui_rounding); // Color button are often in need of some sort of border
    }
...
bool ImGui::TreeNodeBehavior(ImGuiID id, ImGuiTreeNodeFlags flags, const char* label, const char* label_end) {
    ...
    {
        // Framed type
        const ImColor32 bg_col = GetImColor32((held && hovered) ? ImGuiCol_HeaderActive : hovered ? ImGuiCol_HeaderHovered : ImGuiCol_Header);
...
void ImGui::TreePop() {
    ...
    window->DC.TreeDepth--;
    ImColor32 tree_depth_mask = (1 << window->DC.TreeDepth);
...
bool ImGui::Selectable(const char* label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg) {
    ...
    if (hovered || selected)
    {
        const ImColor32 col = GetImColor32(ImGuiCol_HeaderHovered);
...
int ImGui::PlotEx(ImGuiPlotType plot_type, const char* label, float (*values_getter)(void* data, int idx), void* data, int values_count, int values_offset, const char* overlay_text, float scale_min, float scale_max, const ImVec2& size_arg) {
    ...
    if (values_count >= values_count_min)
    {
        ...
        const ImColor32 col_base = GetImColor32((plot_type == ImGuiPlotType_Lines) ? ImGuiCol_PlotLines : ImGuiCol_PlotHistogram);
        const ImColor32 col_hovered = GetImColor32((plot_type == ImGuiPlotType_Lines) ? ImGuiCol_PlotLinesHovered : ImGuiCol_PlotHistogramHovered);
...
bool ImGui::BeginMenuEx(const char* label, const char* icon, bool enabled) {
    ...
    else
    {
        ...
        RenderArrow(window->DrawList, pos + ImVec2(offsets->OffsetMark + extra_w + g.FontSize * 0.30f, 0.0f), GetImColor32(ImGuiCol_Text), ImGuiDir_Right);
    }
    if (!enabled)
        EndDisabled();
...
bool ImGui::MenuItemEx(const char* label, const char* icon, const char* shortcut, bool selected, bool enabled) {
    ...
    else
    {
        ...
        pressed = Selectable("", false, selectable_flags | ImGuiSelectableFlags_SpanAvailWidth | ImGuiSelectableFlags_DontClosePopups, ImVec2(min_w, label_size.y));
        if (g.LastItemData.StatusFlags & ImGuiItemStatusFlags_Visible)
        {
            if (selected) {
                ImVec4 customColor = ImVec4(ClientUtils::clientColor[0], ClientUtils::clientColor[1], ClientUtils::clientColor[2], ClientUtils::clientColor[3]);
                RenderTextColored(pos + ImVec2(offsets->OffsetLabel, 0.0f), label, nullptr, true, customColor);
            } else {
...
namespace ImGui
{
    ...
    static ImColor32            TabBarCalcTabID(ImGuiTabBar* tab_bar, const char* label, ImGuiWindow* docked_window);
...
bool    ImGui::BeginTabBarEx(ImGuiTabBar* tab_bar, const ImRect& tab_bar_bb, ImGuiTabBarFlags flags) {
    ...
    // Draw separator
    const ImColor32 col = GetImColor32((flags & ImGuiTabBarFlags_IsFocused) ? ImGuiCol_TabActive : ImGuiCol_TabUnfocusedActive);
...
static ImColor32   ImGui::TabBarCalcTabID(ImGuiTabBar* tab_bar, const char* label, ImGuiWindow* docked_window) {
...
bool    ImGui::TabItemEx(ImGuiTabBar* tab_bar, const char* label, bool* p_open, ImGuiTabItemFlags flags, ImGuiWindow* docked_window) {
    ...
    const ImColor32 tab_col = GetImColor32((held || hovered) ? ImGuiCol_TabHovered : tab_contents_visible ? (tab_bar_focused ? ImGuiCol_TabActive : ImGuiCol_TabUnfocusedActive) : (tab_bar_focused ? ImGuiCol_Tab : ImGuiCol_TabUnfocused));
...
void ImGui::TabItemBackground(ImDrawList* draw_list, const ImRect& bb, ImGuiTabItemFlags flags, ImColor32 col) {
    // While rendering tabs, we trim 1 pixel off the top of our bounding box so they can fit within a regular frame height while looking "detached" from it.
    ImGuiContext& g = *GImGui;
    const float width = bb.GetWidth();
    IM_UNUSED(flags);
    IM_ASSERT(width > 0.0f);
    const float gui_rounding = ImMax(0.0f, ImMin((flags & ImGuiTabItemFlags_Button) ? g.Style.FrameRounding : g.Style.TabRounding, width * 0.5f - 1.0f));
    const float y1 = bb.Min.y + 1.0f;
    const float y2 = bb.Max.y - 1.0f;
    draw_list->PathLineTo(ImVec2(bb.Min.x, y2));
    draw_list->PathArcToFast(ImVec2(bb.Min.x + gui_rounding, y1 + gui_rounding), gui_rounding, 6, 9);
    draw_list->PathArcToFast(ImVec2(bb.Max.x - gui_rounding, y1 + gui_rounding), gui_rounding, 9, 12);
    draw_list->PathLineTo(ImVec2(bb.Max.x, y2));
    draw_list->PathFillConvex(col);
    if (g.Style.TabBorderSize > 0.0f)
    {
        draw_list->PathLineTo(ImVec2(bb.Min.x + 0.5f, y2));
        draw_list->PathArcToFast(ImVec2(bb.Min.x + gui_rounding + 0.5f, y1 + gui_rounding + 0.5f), gui_rounding, 6, 9);
        draw_list->PathArcToFast(ImVec2(bb.Max.x - gui_rounding - 0.5f, y1 + gui_rounding + 0.5f), gui_rounding, 9, 12);
        draw_list->PathLineTo(ImVec2(bb.Max.x - 0.5f, y2));
        draw_list->PathStroke(GetImColor32(ImGuiCol_Border), 0, g.Style.TabBorderSize);
    }
}
...
void ImGui::TabItemLabelAndCloseButton(ImDrawList* draw_list, const ImRect& bb, ImGuiTabItemFlags flags, ImVec2 frame_padding, const char* label, ImGuiID tab_id, ImGuiID close_button_id, bool is_contents_visible, bool* out_just_closed, bool* out_text_clipped) {
    ...
    else if (unsaved_marker_visible)
    {
        ...
        RenderBullet(draw_list, bullet_bb.GetCenter(), GetImColor32(ImGuiCol_Text));
    }
